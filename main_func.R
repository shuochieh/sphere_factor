...
#' 
main_BWS = function (dta, r, test_size = 0, h = 6, mu_tol = 1e-3) {
  ...
  for (i in 1:r) {
    ...
    transport_matrix = pt_bws(mu_hat, dta$mu, v_to_matrix)
    transported_V[,i] = tangent_in_E(transport_matrix, dta$mu)
    
    subspace_dist[i] = sine_theta(A[,1:i], transported_V[,1:i])
  }
